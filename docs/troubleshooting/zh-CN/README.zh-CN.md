# 问题排查指南

[排查首页](../README.md) | [返回项目 README](../../../README.zh-CN.md) | [English](../en/README.md)

当安装命令、GUI 弹窗、摄像头流程、模型加载、数据集校验或训练运行失败时，从这里开始排查。Vision Workbench 的用户可见错误会尽量附带对应的排查文档路径。

## 按症状选择

| 看到的问题 | 优先查看 |
| --- | --- |
| 命令找不到、Python 导入失败、Tkinter 启动、路径或编码问题 | [环境问题](./environment.zh-CN.md) |
| Torch、torchvision、CUDA、`nvidia-smi`、`doctor` 检查问题 | [深度学习依赖](./deep-learning-dependencies.zh-CN.md) |
| `.pt`/`.pth` 缺失、模型下载失败、checkpoint 错误 | [模型与权重](./models-and-weights.zh-CN.md) |
| 图片打开/保存、JSON 点对、checkpoint、输出文件失败 | [数据与文件](./data-and-files.zh-CN.md) |
| 摄像头扫描/打开/读帧、截图、录屏、权限或占用问题 | [摄像头与视频](./camera-and-video.zh-CN.md) |
| 分类数据集、YOLO `data.yaml`、标签、mask、训练中止 | [数据集与训练](./datasets-and-training.zh-CN.md) |
| 基础 CV、全景重构、检测/分割运行期错误 | [模块运行错误](./module-runtime-errors.zh-CN.md) |
| wheel 构建、`dist/`、发布资产、超过 100 MB 的模型文件 | [打包与发布](./packaging-and-release.zh-CN.md) |

## 快速检查

除非具体文档另有说明，以下命令都在仓库根目录执行：

```bash
python --version
python -m pip --version
python -m pytest
python scripts/install_dependencies.py doctor
```

反馈问题时，请尽量提供完整弹窗或终端输出、执行过的命令、错误提示附带的文档路径，以及当前安装的是基础依赖、图像分类依赖还是 YOLO26 依赖。

